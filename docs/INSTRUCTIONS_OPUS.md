...
- **フルゲート**: `npm test`（全 green・現在1341件/49スイート）、`npm run lint`（**0 errors 維持**。84件の既存 no-console warning は増やさない）、`npm run build`（green）。
...
- **A-1**: `assets/js/`（死コード）と `tests/archive/` の削除
- **A-2**: webpack/TypeScript 系の未使用 devDependencies 削除
