...
- **フルゲート**: `npm test`（全 green・現在1480件/47スイート）、`npm run lint`（**0 errors 維持**。128件の既存 no-console warning は増やさない）、`npm run build`（green）。
...
- ~~`enableWebPanel` 既定値~~ — **Session 74 で true に変更**（false を正当化した実測条件が全て解消されたため）。戻すなら1行だが、どの条件が再発したかを言えること
