...
2. **リーダーはフォームを扱わない**（散文・リンク・表の行・画像の**代替テキストと画素**を表示）。
   ~~画像~~ → **続き18 で情報（alt）、続き20 で画素まで実装**。
   続き18 の問い:「リーダーに必要なのは画像の*画素*か*情報*か」→ **情報**であり
   それは `alt`（**WCAG 1.1.1 = 仕様の最初の達成基準**）。従来は `<img>` が void 要素で
   ブロック走査に見えず、**全ての代替テキストが黙って捨てられていた**。
   **続き20 で自己訂正**: 「汚染 canvas はテクスチャにできないので画素は不可能」という
   ここの記述は**誇張だった** —— 汚染するのは `crossOrigin` 無しの cross-origin 画像だけで、
   `crossOrigin='anonymous'` なら CORS-clean な画像は汚染されない＝**リーダーが markup に
   ついて既に持つのと同じ到達範囲**。1ページ8枚上限・6行予約（既存の行モデルに乗るので
   スクロール/ページ送り/クリップは全て流用）・contain（拡大なし）・失敗は alt に
   フォールバック。**新しいネットワーク面はゼロ**（プロキシの content-type allowlist は
   未変更）。実ブラウザ `verify:vr-boot` で実デコードを検査（ただし同一オリジン配信なので
   `crossOrigin` 属性自体の証明は unit テスト側）。
...
| 2 | ~~リーダーの画像対応~~ — **続き18 で alt、続き20 で画素まで完了**。表も続き18 で完了 | WCAG 1.1.1。フォームは意図的に未対応 |
