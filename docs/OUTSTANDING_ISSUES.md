...
### P-5. 記録（意図的）: IME の `l-` 系は x- 別名（小かな）解釈
続き235 で追加した l- 系は `la`→`ぁ`（Mozc 流儀の小かな接頭辞）。ヘボン式に
`le`→`れ` と期待するユーザーには `le`→`ぇ` と見えるが、本機は日本語 IME
モードとして Mozc/ATOK 系統の解釈を採用。`ww` も意図的に促音化しない
（'www' 笑いが 'っww' に化けるのを防ぐ）。`l`/`w` 単打は未変換のまま残る。
