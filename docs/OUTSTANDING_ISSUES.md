...
3. **欠陥クラスの自動化**: 設定到達性・i18n 網羅・デプロイ設定（3/3 で同じ欠陥が出た「ビルドせず配信」）・
...
2. **リーダーは画像・表・フォームを表示しない**（テキストと リンクのみ）。WebXR ウェブアプリの
   プラットフォーム上限（cross-origin 画素は合成不能）の内側で、画像だけは原理的に可能（CORS/プロキシ経由の
   `<img>` 抽出 + テクスチャ化）だが、メモリ予算と レイアウトの再設計を要する。
...
| 2 | リーダーの画像対応（CORS/プロキシ経由のみ） | 原子②の残り半分。予算: 1ページ N 枚上限 + 遅延読込 |
| 3 | ページ内検索（読み上げ済みテキストへの検索） | 長文記事で視線スクロールの手数を大幅削減 |
...
**Session 75: 3つの「到達性」欠陥クラスを自動化した**。同じ形の欠陥（作られているが誰も到達できない）を手作業で何度も見つけてきたので、走査そのものをテストにした —— ①`tests/settings-reachability.test.js`（全設定キーにコントロールが存在するか。`enableWebPanel`・`enablePerfMonitorUI`・`enableVoice` の3件を当時捕捉できた）②`tests/i18n-coverage.test.js`（src の全 `t()` キーが en/ja 双方で解決し、かつ英語のコピーのままでないか。Sessions 27・74続き8・74続き9 の3件を捕捉できた）。あわせて**コールバック到達性**（コンポーネントが受け取る `on*` 16件・音声コマンドの `on*` 12件）を走査し、**全て配線済み**を確認（現時点で orphan なし）。
