...
## H. ターゲット角サイズ監査（Session 70）— メートルで書かれた寸法は一度も「度」で検証されていなかった
...
### H-2. 未修正（実測済み・要設計）: パネル距離設定がすべてのターゲットを壊す

`WindowManager` は `target.scale` を**一度も触らない**ので、パネルの角サイズは距離に反比例する。
設定ステッパー `vr.settings.panelDist` の範囲は **0.6〜6.0 m（10倍）**。実測:

| 距離 | chrome ボタン | 移動バー(hitslop後) | タブ本体 | ブックマーク行 | 判定 |
|---|---|---|---|---|---|
| 0.6 m（最小） | 10.1° × 7.6° | 43.6° × 3.3° | 12.0° × 6.7° | 86.3° × 8.0° | すべて快適だが**パネル幅が 106°**（快適な中心視野 ~60° の倍） |
| 1.2 m（大文字設定） | 5.1° × 3.8° | 22.6° × 5.0° | 6.0° × 3.3° | 50.2° × 4.0° | 快適 |
| **2.0 m（既定）** | 3.0° × 2.3° | 13.7° × 3.0° | 3.6° × 2.0° | 31.4° × 2.4° | 1.5° は全通過、3° には一部届かず |
| **6.0 m（最大）** | 1.0° × 0.76° | 4.6° × 1.0° | 1.2° × 0.67° | 10.7° × 0.81° | **全ターゲットが 1.5° 未満 = 視線では操作不能** |

**正しい直し方**は角サイズ一定化（`scale = distance / PANEL_DISTANCE_DEFAULT`）。2.0 m で scale 1.0 なので
既定挙動は完全不変にできる。距離を変える正当な理由は**輻輳・調節の眼の快適さ**（Session 62 の調査: 0.5 m 未満/20 m 超を避ける、
近視者は 2.5 m が快適）であって見かけの大きさではないので、角サイズを保ったまま奥行きだけ変わるのが本来の挙動。

**本セッションで着手しない理由（先に解くべき依存）**: タブストリップ（`TabManager.stripGroup`）は
パネルグループの**兄弟**で、固定座標 `this.position` に置かれ `windowManager.target`（= `active.group`）には含まれない。
つまり現状でも grab-to-move でパネルを動かすとストリップだけ取り残される（**既存の別バグ**）。
パネルだけを scale すると乖離が悪化するので、**まずストリップを管理対象の子にする**必要がある。
`tests/target-size.test.js` は上表の「6 m で全滅」を**明示的に固定**しているので、忘却・悪化はしない。
...
- **chrome ボタン（3.0°×2.3°）・リーダー矢印（4.3°×2.0°)・ブックマーク行（31.4°×2.4°）は 2 m で 3° 未満**。
  ...
  レイアウト再設計（H-2 と同時にやるべき）。1.5° フロアは全通過。
