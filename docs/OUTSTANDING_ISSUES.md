...
## P. リーダー抽出器の残存スコープ（続き233 で測定・一部修正）

`src/vr/browser/readableText.js` の実測監査で見つかった欠陥は修正済み
（有名実体全集・大小文字厳密化・`<ol start|reversed|value>` 入れ子計数・
...
テキストオンリー読み物として正）。`<wbr>` も未サポートだが、VR リーダーの
改行エンジン（textWrap）が既に任意幅で折り返すため影響は観測されない。

### P-3. 検証済み・問題なし: `liftUnreachable` の `<table>` 行化
既存の表→行変換は `<tr>` 単位で ` | ` 結合する単純化で、colspan/rowspan を
無視する。これはリーダーの単純化方針として意図的（Mozilla Readability も
表は構造を捨てる）。新たな欠陥ではない。
...
`le`→`れ` と期待するユーザーには `le`→`ぇ` と見えるが、本機は日本語 IME モード
として Mozc/ATOK 系統の解釈を採用。`ww` も意図的に促音化しない（'www' 笑いが
'っww' に化けるのを防ぐ）。`l`/`w` 単打は未変換のまま残る。
