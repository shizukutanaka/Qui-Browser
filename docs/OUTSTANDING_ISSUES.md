...
- ~~**Stop（読み込み中断）**: `loading=true` を解除できるのは onload/onerror のみ~~ — **Session 75 で実装**。`WebPanel.stop()`: reader fetch の AbortController を abort・`_readerSeq++` で in-flight 結果を破棄・iframe の onload/onerror を外して `about:blank` へ（about:blank の load が onNavigate を発火しないよう切断）。ロード中は chrome バーの reload ボタンが ✕ に化け同じ hit zone が stop() に振替（標準ブラウザの chrome 挙動）。描画済み reader コンテンツがあれば blank にせず保持。4テスト追加・pre-fix 失敗確認済み
