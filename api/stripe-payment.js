/**
 * SUPERSEDED — not mounted anywhere, kept only as historical reference.
 *
 * Stripe Payment API
 * Chrome拡張機能使用料の決済処理
 ...
 * @version 2.0.0
 *
 * This described a different, now-abandoned pricing model (a "Chrome
 * extension" license at $0.50/mo or $1.50 lifetime) than the product's real
 * JPY-tiered VR subscription plans. The canonical, actually-wired billing
 * implementation is server/stripe-billing.js, started via server/index.js
 * (`npm run start:server`). This file also calls app.listen() itself, so
 * requiring it alongside server/index.js would bind the same port twice —
