...
- 🧪 **ローカル検証**: 全 YAML parse OK、`npx eslint src/vr --quiet` 0 errors、`npm run test:integration` 305 tests、`npm run ci:verify`（build+layout+app+vr-boot 448 checks）全 PASS、dist/index.html+assets 存在・1.0MB<2MB。ubuntu-latest は `/usr/bin/google-chrome` 同梱で `findChrome` が拾える想定（CI 実行で実証）。
- ✅ ゲート全緑後 push（#285 の pull_request は head の workflow を使うため本 PR が修復の live test になる）。
