...
- 🧪 赤検証: array-pattern for-loop 無効化 → `hapticSequence` FAIL、dedup 逆方向（両手発火）→ `hapticBothHandsDedup` FAIL。全 pin 有機全緑。
- ✅ 3302 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:vr-boot 258 checks PASS、verify:app PASS。
...
### Session 212: 続き370 — LayersSystem 全ライフサイクル e2e pin（#270 batch 29、326→331 checks）

- 🔍 実測: `initialize` の XRWebGLBinding 無し・ctor 投げ両腕 → unsupported、`createQuadLayer` の実引数（space/GL_RGBA8/mono/isStatic:false）+ registry + transform、`renderCanvasToLayer` の per-view subimage blit（bindFb→viewport→bindTex→texSubImage2D）+ finally unbind + 空 views/死 sub スキップ、`updateRenderState` の baseLayer 先頭順 + `removeLayer` 再 commit、`dispose` teardown。全て未駆動 — headless に XRWebGLBinding が無いため全 pin は fresh instance + eval 内 class inject で駆動（app 本体の `app.layersSystem` は supported=false で残存し `.constructor` mint 可能）。
- 🔧 ハーネス教訓: `globalThis.XRWebGLBinding` は finally で xrWas 復元（native 未定義なら delete）。`texSubImage2D` の source 引数は a[6]（a[5]=UNSIGNED_BYTE — 初期 pin は index ずれで偽 FAIL）。render-state の layer 値は XRQuadLayer オブジェクト — join 文字比較不可で要素 identity で検証。
...
### Session 220: 続き378 — teleport impact pulse + invalid-aim no-op + reduced-motion smooth warn を e2e pin（#276、353→356 checks）
- 🔍 **実測（未駆動 3 腕）**: ①`fireTeleportFeedback` の haptic 'impact' 腕（teleportLands は caption のみ観測）。②`updateTeleport` miss → `valid=false`+marker 非表示 → squeezeend no-op（stale target へ動かない契約）未駆動。③`smoothMoveWarning` の OS reduced-motion 連動 warn（settings 唯一の OS signal select 経路）未駆動。
- 🔧 **ハーネス設計・赤検証教訓**: (a) else 腕 `t.valid=false` 切断は `_resetTeleportAim` が毎回 valid をゼロに戻すため不発 — 真の契約は `onTeleportEnd` の `t.active && t.valid && t.target` ゲート（stale `t.target` は reset を生き残る）。(b) `fireTeleportFeedback(null,…)` は handedness `|| 'right'` fallback で pulse 発火し続けるため不発 — haptic 腕は helper 内で切断必須。(c) `matchMedia` は `media()` が live 参照するため eval 内 stub で OS signal を差替可、finally で必ず復元。
...
### Session 232: 続き390 — vrButton guarded handler が実機でも未インストールだった実欠陥を捕捉・fix + prompt fallback 残腕を e2e pin（#280 batch 49、408→417 checks）
- 🔍 **実欠陥（pin が有機 red で捕捉）**: THREE r186 の `VRButton.createButton` は `button.onclick` を `isSessionSupported` 解決後に**非同期で**代入するため、init 時点では常に `null` — `if (typeof vrButton.onclick === 'function')` のガードは実機でも常に false で、guarded handler（pendingRequest dedup・denied 時の error toast・'EXIT VR' ラベル・session-end 復元）は**導入以来一度もインストールされていなかった** 40 行のデッドコード。`String(btn4.onclick).length===185` で THREE 原本の残存を実測。`Object.defineProperty`（getter で guardedClick 返却・setter で THREE の非同期代入を飲む）で確実にインストールするよう fix。
- 🔧 **ハーネス設計・教訓 3 件**: ①`requestSession` の Promise.reject を実駆動すると `captureError→import('@sentry/browser')` が bare specifier で失敗し `console.error` が 'no console errors' check を殺す — **importmap で同一オリジンの harness サーバ stub ルートに解決**（`data:` URL は CSP `script-src 'self'` で拒否されるのが教訓）。②re-issue click は pendingRequest 未解決のまま残る — reject 腕の click が dedup で飲まれ `vrBtnErrorToast` が有機 FAIL → 2 回目も `resolve4` で drain してから reject 腕へ。③fake session の `listeners.end` は単一スロットなので THREE の `onSessionEnd`（`session.removeEventListener` を呼ぶ）が登録されると eval が死ぬ — `removeEventListener`/`end` も実装した完全形 FakeSession に。
- 🧪 赤検証 3 run: ①defineProperty 切断 → endsLive/dedup/reissues/errorToast 4 pin FAIL（install が真の契約 — enters/exitLabel は THREE 原本でも通る想定 co-signal）。②`window.prompt`+`activate` 切断 → kbFallbackConfirm/kbPrefillBlank FAIL。③`if(url)`→`if(true)` + `addEventListener('end')` 切断 → kbFallbackCancel/vrBtnEnters/vrBtnExitLabel FAIL。全 9 pin 個別 falsifiable。
- ✅ 3306 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:vr-boot 417 checks PASS、verify:app PASS。CI 未設定（0/0）。
### Session 233: 続き391 — defineProperty getter は DOM dispatch に効かない実欠陥#2 を捕捉・fix + retry/enter-vr/toast/detach-args を e2e pin（#281 batch 50、417→422 checks）
- 🔍 **実欠陥#2（pin が有機 red で捕捉）**: R150 の `Object.defineProperty(vrButton,'onclick',{get(){return guardedClick}})` は **JS 側の読取りだけ**を差し替える — Blink の `.click()` dispatch は要素の**内部 event-handler スロット**（真の onclick setter/addEventListener が書く所）を参照するため、飲み込まれた THREE 代入が一切触らないスロットは空のまま。`.click()`/`'enter-vr'→click()` は**ハンドラ空打ち — 実機で click→requestSession が全滅**（dedup/retry/toast 全 dead のまま）。`onclick()` 直接呼出しが動くのに `.click()` が 0 call という pin の赤で確定。fix: defineProperty（THREE 非同期代入の飲込み）は維持 + `vrButton.addEventListener('click', guardedClick)` で実 dispatch 経路に登録。
- 🔧 **ハーネス設計・教訓 3 件**: ①`_toastTimers`/camera.children には先行 leg の toast が蓄積（10+ timers, 11 meshes）— 絶対値 assert は破綻するので **`new Set(before)` で差分追跡**（新規 mesh/timer のみ抽出 → expiry で除去を観測）。②dedup が連続 click probe を飲む — `resolve5` 捕捉した pending を **arm 間で drain** してから次の click へ（`s||(s=requestSession…)` の s=null 確認）。③mock 整合性: plain-object の VRButton mock に `addEventListener` が無いと source の listener 登録で setupVR が throw — 実 DOM は常に持つので mock 側に no-op 追加が正当（jest の 4 test 修復）。
- 🧪 赤検証 3 run: ①addEventListener+`_toastTimers.add`+`disableLayerMode(false)` 同時切断 → enterVRClick/toastLifecycle/sessPanelDetach FAIL + sessEnd composite は想定 co-signal（`_onLayerDetach`→fake layersSystem で throw → sessionend 途中死亡 — `false` 引数の契約を強く実証）。②NotSupportedError retry 分岐切断 → vrBtnRetryLocal+vrBtnRetryOnlyNS 両 FAIL（refTypes4 共有の co-signal）。③`else throw err` 飲込み切断 → vrBtnRetryOnlyNS のみ FAIL（独立証明）。全 5 pin falsifiable。
- ✅ 3306 tests / 74 suites 全緑（mock addEventListener 追加で修復）、lint 0 errors（R150 の brace-style 2 error も解消 — `get(){…}` 一行関数を展開）、build 緑、verify:vr-boot 422 checks PASS、verify:app PASS。CI 未設定（0/0）。

### Session 234: 続き392 — hidden-tab video pause・settings drawer 全発火・accordion rebuild/early-return を e2e pin（#283 batch 51、422→427 checks）
- 🔍 **実測（未駆動 4 面）**: ①`onDocumentVisibilityChange`（document の DOM visibilitychange — XRSession 側とは別経路）の `document.hidden && immersiveVideo.playing → togglePause()`。②`_redrawSettingsPanel` の `_settingsPanelDrawers.forEach(fn && fn())` 全 drawer 発火（highContrast apply 側は hcApplied が既 pin）。③`_toggleSettingsSection` の `updateSetting(openSettingsSections)` + `_rebuildSettingsPanel()`（dispose→unregister→再構築）+ 同一 section の early-return。
- 🔧 **ハーネス設計・教訓 3 件**: ①`document.hidden` は Document.prototype getter — `Object.defineProperty(document,'hidden',{configurable:true,get(){…}})` で own-prop shadow + `delete document.hidden` で復元（restore 用 descriptor は存在しない）。②pause 契約は guard 両面を分離 pin — `togglePause()` 切断は pause+gates 両方、`document.hidden &&` 切断は gates のみ FAIL と 2 切断で conjunct 分離。③accordion の unregister 契約は `sp0.traverse` で mesh 収集 → rebuild 後 `app.interactables` 非含有で観測（`_disposeSettingsPanel` の真の契約）— 最後は a11y へ toggle 戻しで panel 状態を default-open に復元。
...
### Session 237: 続き395 — reader-proxy 縦断 7 腕（全タブ伝播・将来タブ継承・invalid warn・clear・fetch URL 合成・repaint ゲート）を e2e pin（#283 batch 54、437→444 checks）
- 🔍 **実測（未駆動 7 腕）**: batch 2 の proxy leg は prompt/persist/toast のみ観測で実 consumer 未駆動 — ①`tabManager.setReaderProxyUrl` の `opts.readerProxyUrl` 更新+全開タブ `panel.setReaderProxyUrl` fan-out、②`new WebPanel({readerProxyUrl: this.opts.readerProxyUrl})` の将来タブ継承（TabManager.js:269）、③`normalizeProxyUrl` invalid 腕 → proxyInvalid warn + 無書込、④whitespace→'' clear → proxyCleared message + settings/tabs 復帰、⑤`readerFetchUrl` の `${base}/fetch?url=${encodeURIComponent(raw)}` 合成と `!base→raw` early return（実 fetch 引数で観測）、⑥`WebPanel.setReaderProxyUrl` の same-value early return + `_contentState==='unavailable'` 限定 repaint ゲートの二腕。
- 🔧 **ハーネス教訓 3 件**: (a) **`wp.navigate` は onNavigate コールバック経由で BookmarkStore 実履歴を書く** — leg の probe 遷移が frecency top-site を乗っ取り、後続 voice 'トップサイトを開く' が偽 URL へ誘導 → leg 内で `wp.onNavigate=()=>{}` stub 必須（さらに `history`/`historyIdx`/`currentUrl`/`currentTitle`/`_contentState` を丸ごと save/restore）。(b) **proxy クリア腕は下流 legs の fetch 経路を変える** — 'nohitword' が proxy 経由→直 fetch に変わり CORS console error → 実入力経路（`_requestReaderProxyInput`+`onTextConfirmed`）で batch 末に proxy を復元。(c) **conjunct pin の same-value 腕は state 順序に敏感** — `same9` を `_contentState='reader'` 下で測ると unavailable ゲートに隠蔽され早 return 切断が不発（赤検証で捕捉→'unavailable' 先行設定に修正）。
- 🧪 赤検証: 切断 run A（VRApp 伝播行+ctor 継承引数+`!out.ok` guard+info toast）→ `proxyPropagates`/`proxyNewTabInherits`/`proxyInvalidWarn`/`proxyClearedMsg` 各 FAIL（proxyToast・console-errors は想定 co-signal）。run B（urlDisplay 合成行+空 base early return+WebPanel same-value+gate）→ `readerProxyFetch`/`webPanelProxyRedraw` FAIL。run C（空 base early return のみ）→ `readerDirectFetch` FAIL。run D/E（same-value / gate 各単独）→ `webPanelProxyRedraw` FAIL（両腕独立 falsify 確認）。
- ✅ 3306 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:vr-boot 444 checks PASS、verify:app PASS。

### Session 238: 続き396 — PerformanceMonitorUI 縦断（render 配線・min/max/avg・alert dedup・dispose）を e2e pin（#284 batch 55、444→448 checks）
- 🔍 **実測（未駆動 4 腕）**: `perfUIApplied` は ON トグル腕のみで全 consumer 未駆動 — ①`render()` の `beginFrame()`/`endFrame(renderer)` → `stats.totalFrames++`・`lastFrameTime`・`metrics.frameTime/drawCalls/triangles.current` 書込、②`updateMetric` の running min/max/avg + `sampleMetrics` の history push + 300 cap、③`checkThresholds` → `addAlert`（frameTime>16 crit / memory>1800 crit / fps<60 crit）と 5s same-message dedup（`count++` で unshift 抑制）、④`dispose()` の DOM detach + `container=null`。
- 🔧 **ハーネス教訓 2 件**: (a) **`checkThresholds` は全部位を同時発火する** — 前 leg の `app.render` が実 fps/memory current を書くため、閾値を 1 系統だけ駆動する場合は `metrics.fps.current=0`/`memory.current=0` をリセット必須（初回 FAIL：余計な FPS alert が混ざり `alerts.length`/`alerts[0]` がずれた）。(b) **ページ側 console.warn はハーネス wrapper が握るため spy 到達不能** — `console.warn=fn` で `warnArgs` が 0 のまま → alert 契約は `alerts`/`alertsGenerated`/`level`/`message`/`count` の実オブジェクト観測に限定（warn 出力行自体は 'no console errors' に非干渉）。
- 🧪 赤検証 4 run: `endFrame(renderer)` 呼出切断 → `perfUiRenderFrames` のみ FAIL、`metric.min` 書込切断 → `perfUiMetricMath` のみ FAIL、`if (recent)` dedup 切断 → `perfUiAlerts` のみ FAIL、`container=null` 切断 → `perfUiDispose` のみ FAIL（removeChild 切断は残存 null 化で不発 — 真の観測契約は null 化側、赤検証で切替確認）。全 pin 完全分離 falsifiable。
- ✅ 3306 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:vr-boot 448 checks PASS、verify:app PASS。#284 に 11 件の CI check が初出（監視中）。

### Session 239: 続き397 — repo 全体で FAIL だった CI workflow 群を Vite/jest 実レイアウトに修復（#285 batch 56）
- 🔍 **実測（8 job FAIL の全原因を特定）**: #284 で CI が初出・8 件 FAIL。各失敗は stale テンプレ由来で本 PR と無関係: ①`test.yml`/`deploy.yml` の `assets/js/vr-*.js` 参照 — Vite 前時代のモジュール一覧で 21 ファイル全て不存在（validate は FAIL、compatibility/security は vacuous PASS）。②ci.yml `format:check` — repo 全体 prettier 非クリーン（harness/CLAUDE.md 含む）で全 PR 恒常 FAIL。③`tests/tier-system-integration.test.js` と `tools/benchmark.js`/`tools/check-performance-regression.js`/`npm run benchmark:all` が全て不存在。④Java 用 `jacoco-badge-generator` が JS repo で `jacoco.csv` 不在→`on-missing-report: fail`。⑤deploy `build` は `find assets/js` で FAIL し `path: '.'` で repo 丸ごと Pages artifact にしていた（dist 未 build）。⑥docker `/health` endpoint は nginx に不存在。jest 本体は CI でも 3306 緑だった。
- 🔧 **修復方針**: ゲート弱体化ではなく stale 参照の実レイアウト更新 — `assets/js/vr-*.js`→`src/vr/**/*.js` 実モジュール一覧、`node -c`（CJS 前提で ESM 不可）→`npx eslint src/vr --quiet`、不存在テスト→`npm run test:integration`（実 script: vr-app-wiring+app-smoke）、不存在 benchmark ツール→perf jest suite の wall-time JSON 化、jacoco badge 削除、docker `/health`→`/`、deploy artifact `'.'`→`dist`+実 build 追加。prettier は repo 全体非クリーン（自差分でも FAIL）のため ESLint/audit と同じ advisory `continue-on-error` へ。
- 🧪 **ローカル検証**: 全 YAML parse OK、`npx eslint src/vr --quiet` 0 errors、`npm run test:integration` 305 tests、`npm run ci:verify`（build+layout+app+vr-boot 448 checks）全 PASS、dist/index.html+assets 存在・1.0MB<2MB。ubuntu-latest は `/usr/bin/google-chrome` 同梱で `findChrome` が拾える想定。
- ⚠️ **push ブロック**: git-manager OAuth token に `workflow` scope が無く `.github/workflows/` の push は GitHub が拒否（`refusing to allow an OAuth App to create or update workflow ... without workflow scope`）。修復 diff は patch としてユーザーへ引渡し — 適用には workflow-scope 権限のある push または GitHub UI からの編集が必要。

### Session 240: 続き398 — controller bridge の source ライフサイクル（connected/disconnected の store・forget・guard）を e2e pin（#285 batch 57、448→451 checks）
- 🔍 **実測（未駆動 3 腕）**: 全 disconnect pin は caption のみ観測で、`connected` の `userData.inputSource = e.data` 保存と `getDeviceName` 呼出、`disconnected` の `controllerInput.forget()` + `inputSource=null`、および forget の `if (userData.inputSource)` guard が全て未 pin — forget を消しても全 pin が緑のままだった（'forgets the input source' とコメントしつつ assert が無かった）。
- 🔧 **pin 設計**: `getDeviceName`/`forget` は instance method のため eval 内で wrap して呼出数を観測（finally で確実に復元）。forget の行動契約は WeakMap 非公開のため `read(src)` の返却 snapshot object identity で代用 — forget 済みなら次 read が新しい state+snapshot を構築し `snapAfter !== snapBefore`、未 forget なら同一 object。`srcBridge = {handedness:'right'}` は gamepad 無しで `_empty` 経路でも `_state` entry が作られるため有効。
- 🧪 **赤検証 3 切断**: ①`inputSource = e.data` 切断 → ctrlConnStores FAIL + store を消費する全 downstream legs が想定 co-signal。②`forget(...)` 切断 → ctrlDiscForget のみ FAIL（完全分離）。③guard を `if (true || ...)` に置換 → ctrlDiscGuarded のみ FAIL（完全分離）。新教訓: `if(true){ /*CUT*/ if(...){` の様な差し込みは brace 不整合で build 自体が死に stale dist が誤診を起こす — guard 切断は条件式の置換で行う。
- ✅ 3306 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:app PASS、verify:vr-boot 451 checks PASS。全 pin 有機全緑（純粋カバレッジ）。

### Session 241: 続き399 — `onSessionChange`→`_saveTabSession`+`_syncPanelLayers` 実配線と `a11y` 委譲 get/set 契約を e2e pin（#285 batch 58、451→453 checks）
- 🔍 **実測（未駆動 2 面）**: ①TabManager の全 mutator（newTab/close/setActive/panel navigate）は `opts.onSessionChange?.()` → `VRApp: { _saveTabSession(); _syncPanelLayers(); }` を発火するが、tab-persist pin は `app._saveTabSession()` 直接呼び、panel-layer pin は `app._syncPanelLayers()` 直接呼び — mutation→callback→両 method の実配線は未 pin。②`app.captionSystem/hapticFeedback/gazeInteraction` の `a11y` への get/set 委譲（C-1 抽出の互換契約）も未 pin — setter 本体を消しても全 pin が緑のまま。
- 🔧 **pin 設計**: ①instance method を eval で wrap して呼出数を観測 — `newTab()` は `setActive` 内 1 回 + tail 1 回 = 各 method 2 回呼出を assert（`===2` で単発切断を検出可能に）。`closeTab` で後片付け。②sentinel を 3 プロパティに assign → `app.a11y.* === sentinel`（書込委譲）と `app.* === sentinel`（読出委譲）の双方向を一体 assert、finally で setter 経由復元（a11y 側も戻る）。
...
### Session 248: 続き406 — monitoring visibility + procedural buffer + custom voice cmd + history edges + tex upload を e2e pin（#286 batch 65、473→478 checks）
- 🔍 **実測（未駆動 5 腕）**: ①monitoring の DOM `visibilitychange` リスナ → `trackEvent('session_resumed')` → gtag — session-end scalars しか観測されていなかった。②`registerProceduralBuffer` の `buffers.set` + `buffersLoaded++` + 同名 dedup 早期 return（synthesize スキップ）。③`registerCommand` — 全 leg が組込コマンドのみで public 拡張 API は未駆動（登録した pattern が transcript に一致し action が pipeline 経由で発火）。④`back()`/`forward()` の `historyIdx` 境界 guard（idx0/末尾で false + `_loadUrl` 不発 — spy で歩行を network から切離）。⑤`_drawContent` 完了時の `contentTex.needsUpdate=true` — `texture.version` が write 時のみ increment するので skip は凍結として観測可。
- 🔧 **ハーネス教訓**: `disposeMonitoring` など module-private 状態は eval 到達不可 → listener 腕のみ dispatch で pin。`wp._loadUrl = spy` の own-property 影で back/forward の実歩行を fetch なしで検証できる。perl `s/.../0;/` は複数行一致で全 needsUpdate を一括切断 — 対象はどれでも FAIL 観測に十分。
- 🧪 赤検証 6 切断: visibilitychange listener 切断 → 本 pin のみ。`buffers.set` 切断 → 本 pin + audio 6 件 co-signal（loadAudio/play 共有・想定）。`commands.set` 切断 → 本 pin + voice 全 18 件 co-signal（registerDefaultCommands 共用・想定）。back/forward guard 切断各々 → 本 pin + forward 側は 'faceA no forward' 1 件 co-signal（edge 到達で announce 分岐変化・想定）。needsUpdate 切断 → 本 pin のみ。全 pin 有機全緑（純粋カバレッジ）。
- ✅ 3306 tests / 74 suites 全緑、lint 0 errors、build 緑、verify:vr-boot 478 checks PASS、verify:app PASS。

### Session 249: 続き407 — device-tier UA map + audio source distance + IME 変換腕 + texture settings/stats を e2e pin（#286 batch 66、478→482 checks）
- 🔍 **実測（未駆動 4 腕）**: ①`DeviceCompatibility._detectTier` の UA→perf-tier マップ（quest3/quest2/quest-pro/pico4/pico-neo3/android-xr + desktop-xr/unknown fallback）— tier が targetFPS と locomotion caps を供給するのに boot 経路のみで全 arm 未駆動。②`_sourceDistance` の実 3-4-5 距離（listener 位置との差分 sqrt — HRTF threshold tier の入力）。③`convertRomajiToHiragana` の sokuon 双腕（重子音 'kitte'→きって + tch arm 'matcha'→まっちゃ）と `convertHiraganaToKatakana`・`getOfflineKanjiCandidates` の hit/miss-echo 腕 — public 変換 API が boot のみで一切未駆動。④`applyTextureSettings` の実フィールド書込（anisotropy/wrapS/magFilter/generateMipmaps + renderer max fallback）と `getMemoryStats` の shape 契約。
- 🔧 **ハーネス教訓（重要・bq 再確認）**: `monUnloadOnce`（onUnload dedup pin）を設計したが実 `pagehide` dispatch は `onPageTeardown`（app.js）→ `vrApp.dispose()` → `gazeInteraction.reticle=null` → 後続 `setEnabled` の unguarded `reticle.visible` が TypeError で eval 途中死 — **真の unload/beforeunload イベントは live-app eval と根本的に非互換**、onUnload/_unloaded dedup は e2e pin 不可として確定。`getOfflineKanjiCandidates` の miss arm は `[]` ではなく入力エコー（`['zzzz']`）を返す — assert は実装 arm の実測値に合わせる（node で事前検証が速い）。`utilizationPercent` は小容量で '0.0' — `>0` ではなく string 一致で pin。
...
### Session 253: 続き411 — vr_start emit + recognition opts + confirm speak + resolve ヒューリスティクス + backgrounded を e2e pin（#286 batch 70、493→498 checks）
- 🔍 **実測（未駆動 5 腕）**: ①`onVRSessionStart` → `trackVRSession('start', {device: report.deviceTier})` → gtag 'vr_start' — UA→tier map は既 pin だったが emit 配線は未観測（`vr_end` は既 pin）。②`initialize()` の `recognition.continuous/interimResults/maxAlternatives` 書込（lang のみ既 pin）。③`processCommand` の `speak(matchedCommand.confirmationText)` — action 後の確認発話。④`resolveInput` の残 heuristic 腕 — localhost:port・IPv4:port→https、大文字 HTTPS:// verbatim、ftp://→null+blocked、NFD→NFC fold。⑤visibility listener の `document.hidden` 分岐 → 'session_backgrounded'（resumed のみ既 pin）。
- 🔧 **ハーネス教訓**: `document.hidden` は own property defineProperty → delete で prototype getter に自然復帰（platform accessor を上書きしても除去すれば元通り）。sessStart leg の sessionstart dispatch を gtag spy で包む既存 leg 最小拡張パターンで `vr_start` を観測（二次 dispatch 不要）。confirmationText pin は独自コマンド登録で駆動 — 組込コマンドの announce 経路と同一 speak 行を共有するため赤検証で 7 件 co-signal は想定。
