...
### Session 144: 続き302 — vrButton「防御済み」onclick がデッドコードだった本物の欠陥を修正 + enter-VR 経路を e2e pin（61→63 checks）
- 🔍 **実測（harness が production 欠陥を露出）**: `VRApp.setupVR` の guarded handler インストールは `if (typeof vrButton.onclick === 'function')` ゲート付き — だが THREE の `VRButton.createButton` は `isSessionSupported().then()` の microtask 内でしか `button.onclick` を代入しないため、**チェック時点では常に null でガードブロック全体がデッドコード**。全ブラウザで非防御の `requestSession().then(onSessionStarted)` が走り、requestSession 拒否（非 XR ブラウザが実際に踏む経路）は catch 不在の unhandled rejection + error toast 不発（WCAG 4.1.3 違反）。verify:vr-boot の新規 xr-enter pin で実証: `unhandledrejection` ×2・toast 0・reqCalls=2（THREE handler 経由）。
- 🔧 **修正**: `vrButton.addEventListener('click', ev => { ev.stopImmediatePropagation(); ...同じ防御フロー... })` を**無条件登録** — listener は後から代入される onclick より常に先に発火するため、THREE の microtask タイミングに依存せず確実に横取りできる（race 耐性）。in-flight dedup・liveSession.end()・setSession NotSupportedError → 'local' フォールバック・`.catch` → console.warn + trackVRError + `t('app.error.enterVRFailed')` error toast は従来の設計をそのまま保持。test mock は onclick プロパティ型だったため listener 対応の `vrButtonStub()` に更新（mock-drift 指摘と同型 — stub が実装を cement していた）。
- 🔧 **gate flake 修正（付随）**: `Log.entryAdded` の 'Failed to load resource' error は期待されるテストトラフィック（存在しない .example ホストへの遷移、proxy テストが設定した死んだ localhost:8787 経由のページ）が遅延 flush するため gate を誤爆していた — `entry.url` が served origin を起点としない resource error を除外し、error 行に url を併記（自 origin の error は従来通り gate）。
- 🧪 赤検証: VRApp.js のみ stash → dist 再構築で `failed enter-VR announced` + `no uncaught exceptions` が両方 FAIL（NotSupportedError の unhandled rejection ×2 を捕捉 — production 欠陥の再現）。復元後 63 checks 全緑。pin は alertEl 後読みではなく showVRToast 呼出ごとの `{msg, alert}` スナップショット（遅延 nav-error toast の上書きに依らない）。なお toast は harness の EN locale で発火したため pin は en/ja 両 catalog 値を受理（VoiceCommands 系の JA pin は i18n を通らない固有 JA 文字列だったため従来通り）。
- ✅ 3288 tests / 74 suites 全緑、lint 0 errors（354 warnings ベースライン）、build 緑、verify:vr-boot 63 checks PASS、verify:app PASS。
