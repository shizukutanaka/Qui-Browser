...
### Session 89: 続き247 — 実 Chromium 起動監査: CSP の死んでいた2件（無効 source・meta 非対応 directive）
- 🔍 **実測（qui-browser-2d-runtime skill + vite preview + CDP）**: 実 Chrome で dist を起動し SW/i18n/a11y/enterVR を実走（全て仕様通り動作、SW activated・manifest 200・lang en→ja・HC class/aria-pressed・noWebXR トースト ja）。console に残っていた2件の警告が実欠陥だった: ①`connect-src 'http://[::1]:*'` — **Chrome が "invalid source … will be ignored" と捨てる**（bracketed IPv6 は host-source として受理されない）。IPv6 loopback を許可する意図で実際は死んでおり、同一ポリシー文字列が **meta + nginx×3 + vercel + netlify の6箇所にコピペ済み**。②`frame-ancestors` — CSP 仕様上 <meta> では**常に無視**される directive を meta に記述（headerless ホスト=GitHub Pages では保護ゼロ）。
- 🔧 **修正**: ①`http://[::1]:*` を全6箇所から削除（`localhost:*`/`127.0.0.1:*` が loopback を実測カバー、identical-CSP pin で対称性維持）。②main.js 冒頭に clickjack ガード追加（`top!==self` かつ `top.location.href` 読取で cross-origin 判定→`documentElement.replaceChildren()` で白紙化 — inline script は script-src 'self' で禁止されるため module 実装; nginx/vercel/netlify は `X-Frame-Options: SAMEORIGIN` 既出なので guard が必要なのは Pages のみ）。
- 🧪 pin 2件（csp-consistency.test.js、全件 stash 検証で pre-fix 赤・post-fix 緑）: 全 directive の source に `[` 非含有、main.js に frame guard 存在。ライブ再確認: [::1] 警告は消え、残存は meta frame-ancestors 通知のみ（既知・対策済み）。
- ✅ 3035 tests / 73 suites 全緑、lint 0 errors（350 warnings）、build 緑（guard は index-*.js に同梱を実測確認）。
